  ...
  const handleAddText = () => {
    if (tempText.trim()) {
      if (editingTextId) {
        setTexts((prev) =>
          prev.map((t) =>
            t.id === editingTextId ? { ...t, text: tempText } : t,
          ),
        );
      } else {
      ...
      >
        ...
        >
          <Text
            style={[
              styles.overlayText,
              { color: item.color, fontSize: item.fontSize },
            ]}
          >
            {item.text}
          </Text>
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView style={styles.header}>
        ...
        <View style={styles.headerRight}>
          <TouchableOpacity
            style={[styles.iconBtn, mode === "draw" && styles.activeIcon]}
            onPress={() => setMode(mode === "draw" ? "none" : "draw")}
          >
            <MaterialIcons
              name="edit"
              size={24}
      ...
      >
        <View
          style={[styles.inputContainer, { marginBottom: insets.bottom + 12 }]}
        >
