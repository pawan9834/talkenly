...
export default function BlockedContactsScreen() {
  ...
  const fetchBlockedUsers = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // 1. Get current user's blocked list
      ...
      // 2. Fetch display names & photos for each blocked phone
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            if (success) {
              setBlockedUsers((prev) => prev.filter((u) => u.phone !== contact.phone));
            } else {
  ...
  const renderItem = ({ item }: { item: BlockedUser }) => (
    <View style={[styles.contactRow, { backgroundColor: colors.cardBg, borderBottomColor: colors.border }]}>
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {loading ? (
        ...
        <View style={styles.centered}>
          ...
          <Text style={[styles.emptyTitle, { color: colors.textPrimary }]}>No blocked contacts</Text>
