...
export default function StarredMessagesScreen() {
  ...
  useEffect(() => {
    ...
    // Use collectionGroup to find all messages across all chats where starredBy contains myPhone
    const unsubscribe = firestore()
      ...
      .onSnapshot(async (snapshot) => {
        const messages = await Promise.all(snapshot.docs.map(async (doc) => {
          ...
          // Fetch chat details to get participant names
          let chatName = 'Private Chat';
          if (chatId) {
            const chatDoc = await firestore().collection('chats').doc(chatId).get();
            if (chatDoc.exists()) {
              const chatData = chatDoc.data();
              const otherParticipants = chatData?.participants?.filter((p: string) => p !== myPhone);
        ...
        // If it fails with "index required", this is expected for fresh projects
        setLoading(false);
      });
  ...
  const renderStarredItem = ({ item }: { item: any }) => (
    <View style={[styles.messageContainer, { backgroundColor: colors.cardBg }]}>
      {/* Header showing which chat it belongs to */}
      <View style={styles.chatHeader}>
        <Ionicons name="person-circle" size={24} color={colors.accent} />
        <Text style={[styles.chatName, { color: colors.textPrimary }]}>{item.chatName}</Text>
      ...
      <View style={[styles.body, { backgroundColor: colors.background }]}>
        {loading ? (
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={[styles.emptyTitle, { color: colors.textPrimary }]}>No starred messages</Text>
...
const styles = StyleSheet.create({
  ...
  bubbleWrapper: {
    marginLeft: -8, // compensate for padding in MessageItem
  },
  dateText: {
    fontSize: 11,
    marginTop: 8,
