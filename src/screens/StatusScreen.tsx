...
export default function StatusScreen() {
  ...
  const [myViewedStatusIds, setMyViewedStatusIds] = useState<Set<string>>(
    new Set(),
  );
  ...
  useEffect(() => {
    const loadStoredState = async () => {
      try {
        ...
        if (storedMyStatusUploaded)
          setMyStatusUploaded(JSON.parse(storedMyStatusUploaded));
        if (currentUser) {
      ...
      try {
        await Promise.all([
          AsyncStorage.setItem(
            "talkenly_my_statuses",
            JSON.stringify(myStatuses),
          ),
          AsyncStorage.setItem(
            "talkenly_my_status_uploaded",
            JSON.stringify(myStatusUploaded),
          ),
        ]);
      } catch (e) {
      }
        ...
        const unsub = firestore()
          ...
          .onSnapshot(
            (snapshot) => {
              ...
              const updatedMyStatuses =
                users.find((u: any) => u.id === myId)?.stories || [];
  ...
  useEffect(() => {
    if (activeStatusIndex !== null) {
      const activeUser =
        activeStatusIndex === -1
          ? { stories: myStatuses }
          : contactsStatuses[activeStatusIndex];
  ...
  const renderStatusItem = React.useCallback(
    ({ item }: { item: any }) => {
      const viewedCount = item.stories.filter((s: any) =>
        myViewedStatusIds.has(s.id),
      ).length;
      ...
      return (
        <TouchableOpacity
          ...
          activeOpacity={0.7}
          onPress={() => {
            const globalIndex = contactsStatuses.findIndex(
              (u) => u.id === item.id,
            );
            if (globalIndex !== -1) {
              const firstUnviewed = item.stories.findIndex(
                (s: any) => !myViewedStatusIds.has(s.id),
              );
        ...
        >
          ...
          <View style={styles.statusInfo}>
            <Text
              style={[styles.statusName, { color: colors.textPrimary }]}
              numberOfLines={1}
            >
              {item.name}
            </Text>
            <Text style={[styles.statusTime, { color: colors.textSecondary }]}>
              {formatStatusTime(
                item.stories[item.stories.length - 1].createdAt,
              )}
            </Text>
          </View>
        </TouchableOpacity>
    ...
    [myViewedStatusIds, colors, contactsStatuses],
  );
  const recentUpdates = contactsStatuses.filter((item) =>
    item.stories.some((s: any) => !myViewedStatusIds.has(s.id)),
  );
  const viewedUpdates = contactsStatuses.filter((item) =>
    item.stories.every((s: any) => myViewedStatusIds.has(s.id)),
  );
  const sections = [
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.statusItem}>
        ...
        >
          ...
          >
            ...
            <Image
              source={{
                uri:
                  myStatusUploaded && myStatuses.length > 0
                    ? myStatuses[myStatuses.length - 1].mediaUri
                    : profilePhoto,
              }}
              style={styles.avatar}
            />
        ...
        >
          <View style={[styles.statusInfo, { borderBottomWidth: 0 }]}>
            ...
            <Text style={[styles.statusTime, { color: colors.textSecondary }]}>
              {myStatusUploaded && myStatuses.length > 0
                ? formatStatusTime(myStatuses[myStatuses.length - 1].createdAt)
            ...
            (() => {
              const activeUser =
                activeStatusIndex === -1
                  ...
                  : contactsStatuses[activeStatusIndex];
              if (!activeUser || !activeUser.stories?.[activeStoryIndex])
                return null;
              const story = activeUser.stories[activeStoryIndex];
              return (
                <View
                ...
                >
                  ...
                  <Image
                    source={{
                      uri:
                        story.mediaUri ||
                        `https://picsum.photos/seed/${story.id}/800/1600`,
                    }}
                  ...
                  <View style={styles.viewerOverlay} pointerEvents="box-none">
                    ...
                    <View style={styles.viewerHeader} pointerEvents="box-none">
                      <TouchableOpacity
                        onPress={() => setActiveStatusIndex(null)}
                      ...
                      <TouchableOpacity
                        style={styles.viewerMoreBtn}
                        onPress={() => setViewerMenuVisible(true)}
                      >
                        <MaterialIcons
                          name="more-vert"
                          size={24}
      ...
      >
        <TouchableOpacity
          ...
          activeOpacity={1}
          onPressOut={() => setUploadOptionsVisible(false)}
        >
          <View
            style={[
              styles.bottomSheet,
                ...
                if (item.id === "camera") {
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() => handleAddStatus(true)}
                    >
                      <MaterialIcons
                        name="camera-alt"
                        size={32}
                        color={isDark ? "#FFF" : "#008080"}
                      />
                      <Text
                        style={{
                          color: isDark ? "#FFF" : "#008080",
