...
export default function MediaLinksDocsScreen() {
  ...
  const [linkMessages, setLinkMessages] = useState<
    { url: string; text: string; time: string }[]
  >([]);
  ...
  const fetchMessages = async (isRefresh = false) => {
    ...
    try {
      ...
      const docs = allMessages.filter(
        (msg) => msg.docUrl || msg.type === "document",
      );
      setDocMessages(docs);
    } catch (error) {
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              onPress={() => {
                const fullUrl = item.url.toLowerCase().startsWith("http")
                  ? item.url
                  : `https://${item.url}`;
            ...
            >
              ...
              <View style={styles.linkText}>
                ...
                <Text
                  style={[styles.linkDate, { color: colors.textSecondary }]}
                >
                  {item.time}
                </Text>
              </View>
