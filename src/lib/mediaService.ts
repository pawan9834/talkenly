...
export const startMediaUploadTask = (chatId: string, asset: ImagePicker.ImagePickerAsset) => {
  const timestamp = Date.now();
  const filename = asset.uri.substring(asset.uri.lastIndexOf('/') + 1) || `${timestamp}`;
...
export const uploadChatMedia = async (
  ...
  try {
    const { task, reference, fullFilename } = startMediaUploadTask(chatId, asset);
